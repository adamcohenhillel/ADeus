...
void SendAudioData(const std::vector<char>& data) {
  ...
  if (curl) {
    ...
    std::string remote_url =
        "https://ftkkhhdltskogoewnwov.supabase.co/functions/v1/"
        "record-audio";  // Replace with your server URL
    ...
    // Direct verbose info to stderr (or another FILE* stream)
...
void Main() {
  printf("Spaces Code version: 0.0.1\r\n");
  ...
  const char* hostname = "ftkkhhdltskogoewnwov.supabase.co";
  ...
  while (true) {
    ...
    if (audio_data.size() >=
        16000 * sizeof(int32_t) * 10) {  // 5 seconds of audio
